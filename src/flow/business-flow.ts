...
const PRESET_CONFIGS: Record<FlowPreset, FlowConfig> = {
  document: {
    name: 'document',
...
export class BusinessFlow {
  ...
  private async processQueue(queue: Array<() => Promise<void>>): Promise<void> {
  ...
  private determineBatchStatus(executions: FlowExecution[]): 'completed' | 'partial' | 'failed' {
  ...
  async exportBatchResults(
    jobId: string,
    format: OutputFormat
  ): Promise<Array<{ executionId: string; output: string }>> {
...
export function createBusinessFlow(llmClient: LLMClient): BusinessFlow {
