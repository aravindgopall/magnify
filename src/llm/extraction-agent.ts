...
import type { DocumentGroup, ExtractedData, Entity, Section, TableData, SubagentResult } from '../types/index.js';
...
export class LLMExtractionAgent {
  ...
  async execute(group: DocumentGroup): Promise<SubagentResult> {
    ...
    try {
      ...
      const extractedData = await this.llmClient.completeWithJSON<ExtractedData>(messages, this.extractionSchema);
  ...
  private prepareContent(group: DocumentGroup): string {
  ...
  private buildMessages(group: DocumentGroup, content: string): LLMMessage[] {
...
export function createLLMExtractionAgent(
  llmClient: LLMClient,
  options?: {
    agentId?: string;
    customPrompt?: string;
