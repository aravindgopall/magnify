...
- Preserve exact technical terms, field names, codes, and specifications`;
  ...
  async execute(config: SQLiteQueryConfig): Promise<SQLiteQueryResult> {
    ...
    this.logger.section('SubAgent Analysis');
    const relevantBySource = await this.processChunksInParallel(allChunks, config.query, extractionType);
    
    const totalRelevant = relevantBySource.fixed.length + relevantBySource.heading.length + relevantBySource.toc.length;
    this.logger.log(`[Query] Found ${totalRelevant} relevant chunks out of ${allChunks.length} (fixed=${relevantBySource.fixed.length}, heading=${relevantBySource.heading.length}, toc=${relevantBySource.toc.length})`);
    ...
    const answer = await this.synthesizeAnswer(config.query, relevantBySource, extractionType);
    
    const allRelevantResults = [...relevantBySource.fixed, ...relevantBySource.heading, ...relevantBySource.toc];
    const sources = allRelevantResults.map(r => ({
      chunkId: r.chunkId,
      title: r.title,
      source: r.source,
  ...
  private async classifyQuery(query: string): Promise<ExtractionType> {
    const prompt = `Classify this query into ONE type. Respond with ONLY the type name (summary, entities, full, or custom).

Types:
- summary: asking for overview, brief, main points
- entities: asking to extract, list, find specific items (names, numbers, codes)
...
Type:`;
  ...
  private async processChunksInParallel(
  ...
  ): Promise<Record<ChunkSource, SubAgentResult[]>> {
    const bySource = {
    ...
    this.logger!.log(`[SubAgents] Chunks per DB: fixed=${bySource.fixed.length}, heading=${bySource.heading.length}, toc=${bySource.toc.length}`);

    const allBatchPromises: Promise<{ source: ChunkSource; results: SubAgentResult[] }>[] = [];
    let batchCounter = 0;

    const allChunkLists = [
      { source: 'fixed' as const, chunks: bySource.fixed },
      { source: 'heading' as const, chunks: bySource.heading },
      { source: 'toc' as const, chunks: bySource.toc },
    ];

    for (const { source, chunks: sourceChunks } of allChunkLists) {
      if (sourceChunks.length === 0) continue;

      const batchSize = Math.ceil(sourceChunks.length / 3);
      const batches = [
        sourceChunks.slice(0, batchSize),
        sourceChunks.slice(batchSize, batchSize * 2),
        sourceChunks.slice(batchSize * 2),
      ].filter(b => b.length > 0);

      this.logger!.log(`[SubAgents] Spawning ${batches.length} subagents for ${source} DB (${sourceChunks.length} chunks)`);

      for (const batch of batches) {
        batchCounter++;
        allBatchPromises.push(
          this.processChunkBatch(batch, query, extractionType, source, `${source}-batch-${batchCounter}`)
            .then(results => ({ source, results }))
        );
      }
    }

    this.logger!.log(`[SubAgents] Total: ${allBatchPromises.length} subagents running in parallel`);

    const allResults = await Promise.all(allBatchPromises);

    const grouped: Record<ChunkSource, SubAgentResult[]> = {
      fixed: [],
      heading: [],
      toc: [],
    };
    ...
    for (const { source, results } of allResults) {
      grouped[source].push(...results.filter(r => r.hasRelevantContext));
    }
      ...
      try {
        const agent = new Agent({
          initialState: {
            ...
            thinkingLevel: 'off',
            tools: [],
            systemPrompt: this.buildSubAgentSystemPrompt(extractionType),
          },
          getApiKey: async () => {
            const apiKey = await authStorage.getApiKey('grid');
            if (!apiKey) throw new Error('Grid API key not found');
            return apiKey;
          },
        });
        ...
        this.logger!.log(`[SubAgent:${agentId}] Analyzing chunk "${chunk.title}" (pages ${chunk.start_page}-${chunk.end_page})`);
        ...
        this.logger!.chunkResult(agentId, chunk.title || 'Untitled', `pages ${chunk.start_page}-${chunk.end_page}`, !isNotRelevant, cleanedContext);
        ...
        this.logger!.error(`[SubAgent:${agentId}] Error processing chunk ${chunk.chunk_id}`, error);
        results.push({
          chunkId: chunk.chunk_id,
          title: chunk.title,
          source,
  ...
  private buildSubAgentSystemPrompt(extractionType: ExtractionType): string {
...
- Contains related but different concepts (e.g., query asks about "X" and section only mentions "Y similar to X")
...
Extract relevant context or respond with "${RELEVANCE_MARKER}" if not relevant.`;
  ...
  private async synthesizeAnswer(
    query: string,
    relevantBySource: Record<ChunkSource, SubAgentResult[]>,
    extractionType: ExtractionType
  ): Promise<string> {
    const BATCH_SIZE = 30;
    const formatInstructions = this.getFormatInstructions(extractionType);

    // ── Pass 1: Synthesize each source DB in batches, then merge per-source ──
    this.logger!.synthesisStep(1, 'start', `fixed=${relevantBySource.fixed.length}, heading=${relevantBySource.heading.length}, toc=${relevantBySource.toc.length}`);

    const sourceSynthesisPromises = (['fixed', 'heading', 'toc'] as ChunkSource[]).map(async (source) => {
      const results = relevantBySource[source];
      if (results.length === 0) {
        this.logger!.synthesisStep(1, source, 'no relevant chunks, skipping');
      ...
      this.logger!.synthesisStep(1, source, `${results.length} chunks in ${batches.length} batch(es)`);

      const batchSummaries = await Promise.all(
        batches.map(async (batch, batchIdx) => {
          const contextSections = batch
            .map((result, idx) => {
              return `[Section ${idx + 1}: ${result.title} (pages ${result.startPage}-${result.endPage})]\n${result.context}\n`;
            })
            .join('\n---\n\n');

          const prompt = `Based on the following document sections from the "${source}" source, synthesize a focused answer to the user's query.
...
Document Sections (${source} source, batch ${batchIdx + 1}/${batches.length}):
${contextSections}
...
Synthesize your answer according to the extraction type format. Only include information present in the sections above.`;
          ...
          this.logger!.synthesisStep(1, source, `batch ${batchIdx + 1}/${batches.length} done`, summary ? summary.length : 0);
          return summary;
        })
      );

      const nonEmptyBatchSummaries = batchSummaries.filter(Boolean) as string[];
      if (nonEmptyBatchSummaries.length === 0) {
        return { source, summary: '' };
      }

      if (nonEmptyBatchSummaries.length === 1) {
        return { source, summary: nonEmptyBatchSummaries[0] };
      }

      const mergePrompt = `Merge these ${nonEmptyBatchSummaries.length} partial summaries from the "${source}" source into one coherent summary. Remove duplicates and organize the information clearly.
...
Partial Summaries:
${nonEmptyBatchSummaries.map((s, i) => `--- Batch ${i + 1} ---\n${s}`).join('\n\n')}

Merged summary:`;

      const merged = await this.runAgentPrompt(mergePrompt);
      this.logger!.synthesisStep(1, source, `merged ${nonEmptyBatchSummaries.length} batches`, merged ? merged.length : 0);
      return { source, summary: merged || '' };
    });

    const sourceSummaries = await Promise.all(sourceSynthesisPromises);

    // ── Pass 2: Merge the 3 source summaries into one final answer ──
    this.logger!.synthesisStep(2, 'start', `merging ${sourceSummaries.filter(s => s.summary).length} source summaries`);

    const nonEmptySummaries = sourceSummaries.filter((s): s is { source: ChunkSource; summary: string } => !!s.summary);
    if (nonEmptySummaries.length === 0) {
      return 'Failed to generate an answer. Please try again.';
    }

    if (nonEmptySummaries.length === 1) {
      this.logger!.synthesisStep(2, 'skip', 'only one source had results, using its summary directly');
      return nonEmptySummaries[0].summary;
    }

    const mergedContext = nonEmptySummaries
      .map(({ source, summary }) => `=== Summary from ${source.toUpperCase()} source ===\n${summary}`)
      .join('\n\n');

    const mergePrompt = `You have ${nonEmptySummaries.length} summaries of the same document, each from a different chunking strategy (fixed-size pages, heading-based sections, table-of-contents sections). Merge them into one coherent, deduplicated answer.
...
Merge these summaries into a single answer. Remove duplicates, resolve any conflicts by preferring the more detailed version, and organize the information clearly. Cite which source each piece of information comes from when relevant.`;
    ...
    if (!answer) {
      this.logger!.error(`[Synthesis] Pass 2: Main agent failed to respond`);
      return nonEmptySummaries.map(s => s.summary).join('\n\n');
    }
...
export function createSQLiteQueryPipeline(): SQLiteQueryPipeline {
  return new SQLiteQueryPipeline();
}
