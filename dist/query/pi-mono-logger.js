...
export class PiMonoLogger {
     ...
     */
    async logLLMCall(agentType, systemPrompt, userPrompt, agentState, durationMs, groupId, groupTitle) {
     ...
     */
    async saveQueryAnswer(config, result, totalDurationMs, mainAgentLLMCallId, mainAgentDurationMs) {
     ...
     */
    async savePiMonoAgentState(callId, agentType, agentState, groupId, groupTitle) {
        // Ensure directory exists before writing (handles race conditions)
        ...
        for (const msg of messages) {
            ...
            if (msg.role === 'assistant') {
                ...
                if (Array.isArray(msg.content)) {
                    const hasPartialToolCall = msg.content.some((c) => c.type === 'toolCall' && c.partialArgs);
     ...
     */
    cleanMessage(msg) {
        const { streaming, partial, delta, contentDelta, partialArgs, ...cleanMsg } = msg;
        // Clean content array if present
        if (Array.isArray(cleanMsg.content)) {
            cleanMsg.content = cleanMsg.content.map((c) => {
     ...
     */
    async loadQueryAnswer(sessionId) {
     ...
     */
    async listRecentAnswers(limit = 20) {
        try {
...
export function getPiMonoLogger() {
    if (!loggerInstance) {
        loggerInstance = new PiMonoLogger();
    }
    return loggerInstance;
}
//# sourceMappingURL=pi-mono-logger.js.map