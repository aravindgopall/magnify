...
export class GroupContextAgent {
    ...
    extractionType;
    customPrompt;
    constructor(extractionType, customPrompt) {
    ...
    buildSystemPrompt(extractionType, customPrompt) {
...
Remember: Extract verbatim. The main agent will handle formatting.`;
     ...
     */
    async analyze(group, query, signal) {
        ...
        const llmCallId = await logger.logLLMCall('sub-agent', this.buildSystemPrompt(this.extractionType, this.customPrompt), prompt, this.agent.state, durationMs, group.id, group.title || `Pages ${group.startPage}-${group.endPage}`);
    ...
    buildAnalysisPrompt(group, groupText, query) {
...
Extract relevant context or respond with "${RELEVANCE_MARKER}" if not relevant.`;
     ...
     */
    extractRelevantContent(rawResponse) {
    ...
    extractExcerpt(context, maxLength = 200) {
