...
- When referencing tables or structured data, maintain accuracy`;
     ...
     */
    getFormatInstructions(extractionType) {
...
- Cite sections when referencing specific information`;
    ...
    async execute(config) {
        ...
        const subAgentResults = await this.processGroupsInParallel(groups, config.query, maxParallel, config.extractionType, config.customPrompt);
        ...
        const synthesisResult = await this.synthesizeAnswer(config.query, relevantResults, config.extractionType);
        ...
        const confidence = Math.min(0.5 + (relevantResults.length / groups.length) * 0.5, 1.0);
        ...
        await logger.saveQueryAnswer(config, result, totalDurationMs, synthesisResult.llmCallId, synthesisResult.durationMs);
        return result;
    }
    /**
     ...
     */
    async processGroupsInParallel(groups, query, maxParallel, extractionType, customPrompt) {
        const results = [];
     ...
     */
    async synthesizeAnswer(query, relevantResults, extractionType) {
        const startTime = Date.now();
        const logger = getPiMonoLogger();
...
Synthesize your answer according to the extraction type format. If you reference specific information, cite the section number.`;
        ...
        const llmCallId = await logger.logLLMCall('main-agent', this.buildMainSystemPrompt(), synthesisPrompt, this.mainAgent.state, durationMs);
 ...
 */
export function createPiMonoQueryAgent(store) {
    return new PiMonoQueryAgent(store);
}
//# sourceMappingURL=pi-mono-agent.js.map